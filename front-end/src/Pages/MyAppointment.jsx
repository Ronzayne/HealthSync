...
const MyAppointment = () => {
  ...
  const getUserAppointments = async () => {
    try {
      ...
      if (data.success) {
        setAppointments(data.appointments.reverse()); // reverse to make the latest appointment appear at the top
  ...
  const appointmentPayStack = async (appointmentId, email) => {
    console.log("Sending Payment Request:", { appointmentId, email });
    if (!token) {
      return toast.warn("You must be logged in to make a payment");
    }
    try {
      const { data } = await axios.post(
          ...
          >
            ...
            <div className="flex flex-col gap-2 justify-end">
              {!item.cancelled && (
                <button
                  onClick={() =>
                    appointmentPayStack(item._id, item.userData.email)
                  }
